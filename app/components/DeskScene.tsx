...
type Props={pressed:Set<string>;typed:string;active:string;passage:string;difficulty:Difficulty;minutes:number;phase:GamePhase;secondsLeft:number;accuracy:number;onKey:(code:string,label:string)=>void;onDifficulty:(value:Difficulty)=>void;onMinutes:(value:number)=>void;onStart:()=>void;onReset:()=>void};
export function DeskScene({pressed,typed,active,onKey,...game}:Props){
  return <><color attach="background" args={["#f7cfa7"]}/><fog attach="fog" args={["#f7cfa7",12,24]}/><ambientLight intensity={1.6}/><directionalLight position={[-5,8,6]} intensity={3.2} color="#fff1d8" castShadow shadow-mapSize={[2048,2048]}/><pointLight position={[5,1,3]} intensity={1.4} color="#ffd08c"/>
    <Float speed={1.1} rotationIntensity={.008} floatIntensity={0}><group position={[0,0,.72]} rotation={[0,.04,0]}><Monitor typed={typed} active={active} {...game}/><Keyboard pressed={pressed} onKey={onKey}/><Mouse/></group></Float>
