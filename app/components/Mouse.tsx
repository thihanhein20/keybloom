...
export function Mouse({darkMode}:{darkMode:boolean}){
  ...
  return <group ref={ref} position={[4.55,-1.57,1.36]} rotation={[-.14,.30,.025]} scale={.66} onPointerUp={release} onPointerLeave={release}>
    ...
    <RoundedBox args={[.61,.10,.68]} radius={.07} smoothness={4} position={[-.33,button==="left"?.32:.35,-.37]} onPointerDown={(e)=>{e.stopPropagation();setButton("left")}} castShadow><meshStandardMaterial color={darkMode?"#b190aa":"#edc3a5"} roughness={.5}/></RoundedBox>
    <RoundedBox args={[.61,.10,.68]} radius={.07} smoothness={4} position={[.33,button==="right"?.32:.35,-.37]} onPointerDown={(e)=>{e.stopPropagation();setButton("right")}} castShadow><meshStandardMaterial color={darkMode?"#b190aa":"#edc3a5"} roughness={.5}/></RoundedBox>
