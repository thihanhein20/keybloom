...
export function Mouse(){
  ...
  useFrame((state,delta)=>{ref.current.position.x=THREE.MathUtils.damp(ref.current.position.x,4.35+state.pointer.x*.06,8,delta);ref.current.position.z=THREE.MathUtils.damp(ref.current.position.z,2.95-state.pointer.y*.05,8,delta);ref.current.rotation.y=THREE.MathUtils.damp(ref.current.rotation.y,-.06+state.pointer.x*-.02,8,delta)});
  const release=()=>setButton(null);
  return <group ref={ref} position={[4.35,-1.28,2.95]} rotation={[-.10,-.06,0]} scale={.66} onPointerUp={release} onPointerLeave={release}>
