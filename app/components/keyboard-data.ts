export type KeyDefinition = { label: string; code: string; width?: number; color?: string };

export const KEY_ROWS: KeyDefinition[][] = [
  ["Esc","1","2","3","4","5","6","7","8","9","0","-","=","⌫"].map((label,index)=>({ label, code:index===0?"Escape":index===13?"Backspace":`Digit${label}`.replace("Digit-","Minus").replace("Digit=","Equal"), width:index===13?1.55:1, color:index===0?"#8fc9bc":undefined })),
